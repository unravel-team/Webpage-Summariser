...
let globalLiteRtLmPath = undefined;
/**
 ...
 */
export function loadLiteRtLm(path, options) {
 ...
 * module.
 */
export function unloadLiteRtLm() {
    if (hasGlobalLiteRtLmPromise() && !hasGlobalLiteRtLm()) {
        throw new Error('LiteRT-LM is loading and can not be unloaded or canceled ' +
 ...
 */
export function getOrLoadGlobalLiteRtLm(path) {
    const liteRtLmPromise = getGlobalLiteRtLmPromise();
    if (liteRtLmPromise) {
        if (path && globalLiteRtLmPath !== path) {
            throw new Error(`LiteRT-LM is already loading / loaded with a different path ` +
...
//# sourceMappingURL=load_litertlm.js.map