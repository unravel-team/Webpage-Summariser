        ...
        if (WASM_FEATURE_VALUES.threads === undefined) {
            try {
                ...
                WASM_FEATURE_VALUES.threads =
                    Promise.resolve({ supported: false, error: e });
            }
        }
 ...
 *     false otherwise.
 */
export async function supportsFeature(feature) {
 ...
 */
export async function throwIfFeatureNotSupported(feature) {
    const check = WASM_FEATURE_CHECKS[feature]?.();
    if (!check) {
        // Then we don't know how to check for this feature.
        throw new Error(`Unknown feature: ${feature}`);
...
//# sourceMappingURL=wasm_feature_detect.js.map